    ...
    async generate() {
        ...
        try {
            ...
            if (window.canvasInstance && window.canvasInstance.getSelectedImage) {
                ...
                if (selectedImage) {
                    // selectedImage is a Konva.Image node
                    // Get the actual HTMLImageElement from Konva node
                    let sourceImage = null;
                    if (typeof selectedImage.image === 'function') {
                        sourceImage = selectedImage.image();
                    } else if (selectedImage.attrs && selectedImage.attrs.image) {
                        sourceImage = selectedImage.attrs.image;
                    }

                    // Get dimensions from Konva node
                    let width = null;
                    let height = null;
                    if (typeof selectedImage.width === 'function') {
                        width = selectedImage.width();
                    }
                    if (typeof selectedImage.height === 'function') {
                    ...
                    if (sourceImage && width && height) {
                        try {
                            ...
                            console.log(`I2I mode enabled (denoise: ${generationData.denoise})`);
                        } catch (err) {
                            console.error('Failed to convert canvas image:', err);
                        }
                    }
                }
            }
