...
def draw_handpose(canvas, all_hand_peaks):
    ...
    for i in range(len(all_hand_peaks)):
        peaks = np.array(all_hand_peaks[i])
        ...
        for ie, e in enumerate(edges):
            ...
            if x1 > eps and y1 > eps and x2 > eps and y2 > eps:
                if use_matplotlib:
                    color = (matplotlib.colors.hsv_to_rgb([ie / float(len(edges)), 1.0, 1.0]) * 255).astype(np.uint8)
                    color = (int(color[2]), int(color[1]), int(color[0]))  # convert to BGR ints
                else:
                    # Fallback color palette without matplotlib
                    palette = [
                        (255, 0, 0), (255, 85, 0), (255, 170, 0), (255, 255, 0),
                        (170, 255, 0), (85, 255, 0), (0, 255, 0), (0, 255, 85),
                        (0, 255, 170), (0, 255, 255), (0, 170, 255), (0, 85, 255),
                        (0, 0, 255), (85, 0, 255), (170, 0, 255), (255, 0, 255),
                        (255, 0, 170), (255, 0, 85), (200, 200, 200), (150, 150, 150)
                    ]
                    color = palette[ie % len(palette)]
